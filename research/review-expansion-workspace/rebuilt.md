...
- **PDF 内容校验**：120 份来源中，118 份 PDF/网页内容与预期一致；**2 份存在内容不匹配**：
  - S003（OpenClaw 安全分析）：下载的 PDF 为 MIT 阅读时间论文，已用 S004/S031/evidence-matrix 交叉引用重建
...
| 框架 | 核心判断 | 关键支撑数据 | 证据强度 |
|---|---|---|---|
| **OpenClaw** | 混合搜索架构在功能丰富度上领先，但延迟和安全性是明显短板 | 19.6s 召回延迟；CVSS 9.8；ClawHub 26% Skill 漏洞率 | 强（S027 同基准；S003/S031 安全分析） |
...
**安全风险**：OpenClaw 存在已知 CVE-2026-25253（CVSS 9.8），WebSocket gatewayUrl 操纵可导致 token 外泄和一键 RCE。API 密钥和中间推理痕迹以明文存储于 Markdown/SQLite。ClawHub 中被曝 26% 的 Skill 存在漏洞，可向向量数据库写入持久化后门规则 [[S003]](wiki/sources/S003.md) [[S031]](wiki/sources/S031.md)。
...
- [[S003]](wiki/sources/S003.md) OpenClaw 安全分析（CVE-2026-25253）
...
- [[S028]](wiki/sources/S028.md) Business20Channel: 多智能体协调基准
- [[S031]](wiki/sources/S031.md) OpenClaw ClawHub 安全分析
